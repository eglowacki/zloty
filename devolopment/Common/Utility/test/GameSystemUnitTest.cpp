...
TEST(PayloadStager)
{
    ...
    jobPool.AddTask([&stager, &quit, &payloadsConsumed]()
    {
        while (!quit)
        {
            if (auto payload = stager.ConsumePayload())
            {
                payloadsConsumed++;
...
TEST(GameSystem)
{
    ...
    comp::PhysicsWorldComponent* physicsWorld = coordinator.AddCompponent<comp::PhysicsWorldComponent>(physWorldId);
    ...
    for (int i = 0; i < kMaxItems; ++i)
    {
        ...
        comp::LocationComponent* locationComponent = coordinator.AddCompponent<comp::LocationComponent>(nextId);
        coordinator.AddCompponent<comp::PhysicsComponent>(nextId, physicsWorld, physics::BoxCollisionShape(math3d::Vector3(10, 10, 0.5f), 0.0f), locationComponent);
        coordinator.AddCompponent<DummyComp>(nextId);
        coordinator.AddCompponent<DummyComp2>(nextId);
    }
    ...
    for (auto id : ids)
    {
        coordinator.RemoveCompponents(id);
    }

    coordinator.RemoveCompponents(physWorldId);
}
