...
namespace
{
    ...
    {
        static yaget::Strings Min() { return { }; }
        static yaget::Strings Max() { return { "Element1" }; }
        static yaget::Strings Init() { return { "Element1", "Element2" }; }
    };
    ...
    void VerifyConversion()
    {
        ...
        EXPECT_EQ(expectedValue, convertedValue);
    ...
    template<>
    void VerifyConversion<const char*, Limits<const char*>>()
    {
        using T = const char*;
        using Limit = Limits<const char*>;
        T expectedValue = Limit::Init();
        std::string toString = yaget::conv::Convertor<T>::ToString(expectedValue);
        auto convertedValue = yaget::conv::Convertor<T>::FromString(toString.c_str());
        EXPECT_STREQ(expectedValue, convertedValue);
...
TEST(YagetCore, Util_StringConverters)
{
