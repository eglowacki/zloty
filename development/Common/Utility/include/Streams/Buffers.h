...
namespace yaget::io
{
    ...
    struct MessagingBuffer
    {
        ...
        void AssureWriteSize(size_t additionalSize)
        {
            ...
            if (mWriteOffset + additionalSize > currentCapacity)
            {
                ...
                mBuffer = ResizeBuffer(mBuffer, nextBufferSize);
            }
        }
        ...
        void WriteDataChunk(const Buffer& buffer)
        {
    ...
    struct Tag
    {
        ...
        uint64_t Hash() const
        {
            constexpr std::hash<Guid> hasher;
            return hasher(mGuid);
        }
        ...
        bool IsValid() const { return mGuid.IsValid(); }
