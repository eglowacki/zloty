...
static void /*__cdecl*/ odprintf(FILE *pFile, const char *format, ...)
{
    char    buf[4096], *p = buf;
    va_list args;
    int     n;
    ...
    if (pFile != NULL)
    {
        _ftprintf(pFile,  _T("%s"), buf);  // also ok after year 2038 (asctime is NOT ok)
    }
    if (g_debugOutput)
    {
...
static ULONG AllocHashDeinit()
{
    ...
    odprintf(g_fFile, "##### Memory used: #########################################\n");
    AllocHashOut(g_fFile);
#endif // HASH_ENTRY_REMOVE_AT_FREE
    ...
    if (g_CallstackOutputType == ACOutput_Advanced)
    {
        ...
        odprintf(g_fFile, "***** Hash-Table statistics:\n");
        odprintf(g_fFile, "      Table-Size:     %i\n", ALLOC_HASH_ENTRIES);
        odprintf(g_fFile, "      Inserts:        %i\n", AllocHashEntries);
        odprintf(g_fFile, "      Freed:          %i\n", AllocHashFreed);
        odprintf(g_fFile, "      Sum Collisions: %i\n", AllocHashCollisions);
        odprintf(g_fFile, "\n");
        odprintf(g_fFile, "      Max used:       %i\n", AllocHashMaxUsed);
        odprintf(g_fFile, "      Max Collisions: %i\n", AllocHashMaxCollisions);
    }
...
#else // HASH_ENTRY_REMOVE_AT_FREE
                       ...
                       const unsigned char * szFileName, int nLine)
{
    ...
    if (g_ulShowStackAtAlloc > 0)
    {
        odprintf(g_fFile, "##### Memory operation: %s a %d-byte '%s' block (# %ld)", operation[nAllocType], nSize, blockType[_BLOCK_TYPE(nBlockUse)], lRequest );
        if (pvData != NULL)
        {
            odprintf(g_fFile, " at 0x%X", pvData );
        }
        odprintf(g_fFile, "\n");
    }
    ...
    if (nAllocType == _HOOK_FREE) // freeing:
    {
        if (lRequest != 0)  // RequestID was found
        {
            ...
            if (g_ulShowStackAtAlloc > 0)
            {
                ...
                {
                    // RequestID not found!
                    odprintf(g_fFile, "###### RequestID not found in hash table for FREEING (%i)!\n", lRequest);
                }
            }
        }
        else
        {
            if (g_ulShowStackAtAlloc > 0)
            {
                ...
                odprintf(g_fFile, "###### No valid RequestID for FREEING! (0x%X)\n", pvData);
    ...
    if (nAllocType == _HOOK_REALLOC) // re-allocating
    {
        ...
        if (_CrtIsValidHeapPointer(pvData))  // it is a valid Heap-Pointer
        {
            ...
            if (g_ulShowStackAtAlloc > 0)
            {
                ...
                {
                    // RequestID not found!
                    odprintf(g_fFile, "###### RequestID not found in hash table for RE-ALLOCATING (%i)!\n", lReallocRequest);
                }
                else
                {
                    odprintf(g_fFile, "##### Implicit freeing because of re-allocation (# old: %ld, new: %ld)\n", lReallocRequest, lRequest);
                }
            }
        }
    }
    ...
    HANDLE hThread;
    if (DuplicateHandle(GetCurrentProcess(), GetCurrentThread(), GetCurrentProcess(), &hThread, 0, false, DUPLICATE_SAME_ACCESS ) == 0)
    {
        // Something was wrong...
        odprintf(g_fFile, "###### Could not call 'DuplicateHandle' successfully\n");
        InterlockedDecrement(&g_lMallocCalled);
        // call the previous alloc hook
        if (pfnOldCrtAllocHook != NULL)
        {
...
// SymLoadModule64()
typedef DWORD (__stdcall *tSLM)(IN HANDLE hProcess, IN HANDLE hFile, IN PSTR ImageName, IN PSTR ModuleName, IN DWORD64 BaseOfDll, IN DWORD SizeOfDll);
tSLM pSLM = NULL;
...
static bool GetModuleListPSAPI(ModuleList &modules, DWORD /*pid*/, HANDLE hProcess, FILE *fLogFile)
{
    ...
    {
        odprintf(fLogFile, "%lu: EPM failed, GetLastError = %lu\n", g_dwShowCount, gle );
        goto cleanup;
    }

    if (cbNeeded > TTBUFLEN)
    {
        odprintf(fLogFile, "%lu: More than %lu module handles. Huh?\n", g_dwShowCount, lenof( hMods ) );
        goto cleanup;
    }
...
int InitAllocCheck(BOOL debugOutput, BOOL fileOutput, eAllocCheckOutput eOutput, BOOL bSetUnhandledExeptionFilter, ULONG ulShowStackAtAlloc)  // will create the filename by itself
{
    g_debugOutput = debugOutput;
    TCHAR szModName[_MAX_PATH] = {0};
    //if (fileOutput)
    {
    ...
    int result = InitAllocCheckWN(eOutput, fileOutput ? szModName : 0, ulShowStackAtAlloc);
    odprintf(g_fFile, "##### Yaget Memory Leaker version: %d\n", YAGET_LEAK_DETECTOR_VERSION);
    odprintf(g_fFile, "##### Output to Debug: %s and File: %s (%s).\n", g_debugOutput ? "Yes" : "No", fileOutput ? "Yes" : "No", szModName);
    return result;
}
...
DWORD StackwalkFilter(EXCEPTION_POINTERS *ep, DWORD status, LPCTSTR pszLogFile)
{
    ...
    odprintf(fFile, "######## EXCEPTION: 0x%8.8X at address: 0x%8.8X", ep->ExceptionRecord->ExceptionCode, ep->ExceptionRecord->ExceptionAddress);
    odprintf(fFile, ": %s %s\n", GetExpectionCodeText(ep->ExceptionRecord->ExceptionCode), GetAdditionalExpectionCodeText(ep->ExceptionRecord));
...
static void ShowStackRM(HANDLE hThread, CONTEXT& c, FILE *fLogFile, PREAD_PROCESS_MEMORY_ROUTINE64 ReadMemoryFunction, HANDLE hSWProcess)
{
    ...
    if (g_bInitialized == FALSE)
    {
        // Could not init!!!!
        bFirstTime = FALSE;
        odprintf(fLogFile, "%lu: Stackwalker not initialized (or was not able to initialize)!\n", g_dwShowCount);
        return;
    }
    ...
    odprintf(fLogFile, "%lu: ", g_dwShowCount);
    ...
    if (bFirstTime)
    {
        ...
        {
            odprintf(fLogFile, "%lu: SymInitialize(): GetLastError = %lu\n", g_dwShowCount, gle);
            if (tt)
                free(tt);
            goto cleanup;
        }
    ...
    for (frameNum = 0; ; ++ frameNum)
    {
        ...
        if (s.AddrPC.Offset == 0)
        {
            ...
            switch (g_CallstackOutputType)
            {
                case ACOutput_Simple:
                    odprintf(fLogFile, "%lu: (-nosymbols- PC == 0)\n", g_dwShowCount);
                    break;
                case ACOutput_Advanced:
                    odprintf(fLogFile, "   (-nosymbols- PC == 0)\n");
                    break;
            }
        }
        else
        {
            ...
            if (!pSGSFA( hProcess, s.AddrPC.Offset, &offsetFromSymbol, pSym))
            {
                ...
                {
                    if (gle != 487)
                        odprintf(fLogFile, "   SymGetSymFromAddr(): GetLastError = %lu\n", gle);
                    else
                        odprintf(fLogFile, "\n");
                }
            }
            ...
            { // yes, we have SymGetLineFromAddr()
                if (!pSGLFA(hProcess, s.AddrPC.Offset, &offsetFromLine, &Line))
                {
                    ...
                    {
                        odprintf(fLogFile, "%lu: SymGetLineFromAddr(): GetLastError = %lu\n", g_dwShowCount, gle);
                    }
                }
                else
                {
                    ...
                    {
                        case ACOutput_Advanced:
                            odprintf(fLogFile, "%s(%lu) : %+ld bytes\n", Line.FileName, Line.LineNumber, offsetFromLine );
                            break;
                        case ACOutput_Simple:
                            //odprintf(fLogFile, "%lu: %s(%lu) %+ld bytes (%s)\n", g_dwShowCount, Line.FileName, Line.LineNumber, offsetFromLine, undName);
                            odprintf(fLogFile, "%s(%lu) : %+ld bytes (%s)\n", Line.FileName, Line.LineNumber, offsetFromLine, undName);
                            break;
                    }
                }
            }
            ...
            if (g_CallstackOutputType == ACOutput_Advanced)
            {
                ...
                {
                    if (g_CallstackOutputType == ACOutput_Advanced)
                        odprintf(fLogFile, "%lu: SymGetModuleInfo: GetLastError = %lu\n", g_dwShowCount, gle);
                }
                else
                { // got module info OK
                    char ty[80];
                    ...
                    if (g_CallstackOutputType == ACOutput_Advanced)
                    {
                        odprintf(fLogFile, "%lu:     Mod:  %s, base: %08lxh\n", g_dwShowCount, Module.ModuleName, Module.BaseOfImage );
                        if (Module.SymType == SymNone) // Gebe nur aus, wenn keine Symbole vorhanden sind!
                        {
                            odprintf(fLogFile, "%lu:     Offset: 0x%8.8x\n", g_dwShowCount, s.AddrPC.Offset);
                            odprintf(fLogFile, "%lu:     Sym:  type: %s, file: %s\n", g_dwShowCount, ty, Module.LoadedImageName );
                        }
                    }
                }
            }              
