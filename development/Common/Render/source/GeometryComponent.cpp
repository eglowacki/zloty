...
{
    mDevice.ActivatedResource(nullptr, std::format("    Geometry '{}' Started. This: {}", static_cast<comp::Id_t>(Id()), static_cast<void*>(this)).c_str());
...
void yaget::render::GeometryComponent::onUpdateGui(UpdateGuiType updateGuiType)
{
    ...
    if (updateGuiType == Component::UpdateGuiType::Default)
    {
        ...
        bool visible = ImGui::TreeNode(std::format("Entity Id: '{}'", static_cast<comp::Id_t>(Id())).c_str());
        std::string locText = conv::Convertor<math3d::Vector3>::ToString(loc);
        ImGui::SameLine();
        yaget::gui::Text(std::format("Position: '{}'", locText), math3d::Color{ 0.6f, 0.6f, 0.6f, 1.0f });
