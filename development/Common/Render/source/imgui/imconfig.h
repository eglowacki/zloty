...
#define IM_VEC2_CLASS_EXTRA                                                             \
        ...
        operator math3d::Vector2() const { return math3d::Vector2(x, y); }              \
        template <typename TX, typename TY>                                             \
        ImVec2(TX xv, TY yv) { x = static_cast<float>(xv); y = static_cast<float>(yv); }

#define IM_VEC4_CLASS_EXTRA                                                             \
        ImVec4(const math3d::Vector4& f) { x = f.x; y = f.y; z = f.z; w = f.w; }        \
        operator math3d::Vector4() const { return math3d::Vector4(x, y, z, w); }        \
        ImVec4(const math3d::Color& f) { x = f.x; y = f.y; z = f.z; w = f.w; }          \
        operator math3d::Color() const { return math3d::Color(x, y, z, w); }            \
        ImVec4(const colors::Color& f) { x = f.x; y = f.y; z = f.z; w = f.w; }          \
        template <typename TX, typename TY, typename TZ, typename TW>                   \
        ImVec4(TX xv, TY yv, TZ zv, TW wv) { x = static_cast<float>(xv); y = static_cast<float>(yv); z = static_cast<float>(zv); w = static_cast<float>(wv); }
