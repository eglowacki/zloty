...
namespace yaget::render
{
    ...
    enum class AssetCacheType : uint64_t
    {
        ...
        RasterizerStateNone             = 1ULL << 27,
        RasterizerStateClockwise        = 1ULL << 28,
        RasterizerStateCounterClockwise = 1ULL << 29,
        RasterizerStateWireframe        = 1ULL << 30,
        //RasterizerState...            = 1ULL << 31,

        // describe pipeline state depth stencil, uses MaskDepthState
        DepthStateNone      = 1ULL << 32,
        DepthStateOn        = 1ULL << 33,
        DepthStateRead      = 1ULL << 34,
        DepthStencilStateOn = 1ULL << 35,
        //DepthState...     = 1ULL << 36,
        //DepthState...     = 1ULL << 37,
        //DepthState...     = 1ULL << 38,
        //DepthState...     = 1ULL << 39,
        ...
        TopologyStatePoint    = 1ULL << 40,
        TopologyStateLine     = 1ULL << 41,
        TopologyStateTriangle = 1ULL << 42,
        //TopologyState...    = 1ULL << 43,
        //TopologyState...    = 1ULL << 44,

        // how many render targets we want to create
        NumRTVTargetsOne   = 1ULL << 45,
        NumRTVTargetsTwo   = 1ULL << 46,
        NumRTVTargetsThree = 1ULL << 47,
        NumRTVTargetsFour  = 1ULL << 48,
        //NumRTVTargets... = 1ULL << 49,

        // describe pipeline state render target formats
        RTVFormatRGBA8   = 1ULL << 50,
        RTVFormatRGBA16F = 1ULL << 51,
        RTVFormatRGBA32F = 1ULL << 52,
        DSVFormatD24S8   = 1ULL << 53,
        DSVFormatD32F    = 1ULL << 54,
        //DSVFormat...    = 1ULL << 55,
        //DSVFormat...    = 1ULL << 56,
        //DSVFormat...    = 1ULL << 57,
        //DSVFormat...    = 1ULL << 58,
...
//----------------------------------------------------------------------------------------------------------------------------------
template<>
struct yaget::conv::Convertor<yaget::render::AssetCacheType>
{
    static yaget::render::AssetCacheType FromString(const char* value)
    {
        render::AssetCacheType result{};

        auto bitValues = conv::Split(value, "|", true);
        for (auto bit : bitValues)
        {
            auto enumBit = magic_enum::enum_cast<render::AssetCacheType>(bit);
            if (enumBit.has_value())
            {
                result = result | enumBit.value();
            }
    ...
    static std::string ToString(const yaget::render::AssetCacheType& value)
    {
        std::string result;
